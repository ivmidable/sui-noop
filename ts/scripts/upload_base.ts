...
(async () => {
  try {
    //let bytes = fs.readFileSync("../files/basecamp.mp4");
   
    //let digest = await uploadPush("basecamp.mp4", "video/mp4", "binary", bytes);
    await sleep(200);
    let res = await downloadAndSave("../files_downloaded/", "J8tiQCqA5N7oJJ7EGNhNyqSJ41T6nVNGEr3kkREvhELy");
    openFile(res);
} catch (e) {
...
async function uploadPush(name:string, mime:string, encoding:string, img:Buffer) {
